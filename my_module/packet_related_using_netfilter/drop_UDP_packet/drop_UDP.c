...
static const struct file_operations drop_count_ops =
{
    .owner = THIS_MODULE,
...
static int __init vincent_drop_udp_init(void)
{
    ...
    drop_count_file = proc_create("drop_count", 0644, drop_udp_dir, &drop_count_ops);
    ...
    port_file = proc_create("drop_port", 0666, drop_udp_dir, &port_ops);
